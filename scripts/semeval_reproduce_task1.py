...
from ambra.grid_search import GridSearchCV
...
pipe = PossiblePipeline([('vect', Proj(LengthFeatures(), key='lemmas')),
                         ('scale', StandardScaler(with_mean=False,
                                                  with_std=True)),
                         ('clf', IntervalLogisticRegression(C=1e-05,
                                                            n_neighbors=10,
                                                            limit_pairs=1,
                                                            random_state=0))])
...
pipe = PossiblePipeline([('vect', union),
                         ('scale', StandardScaler(with_mean=False,
                                                  with_std=True)),
                         ('clf', IntervalLogisticRegression(C=0.00718685673,
                                                            n_neighbors=10,
                                                            limit_pairs=1,
                                                            random_state=0))])
...
final_pipe.set_params(**{'union__tokens__transf__min_df': 10,
                         'union__tokens__transf__max_df': 0.9,
                         'union__pos__transf__analyzer__ngram_range': (2, 3),
                         'union__pos__transf__max_df': 1.0,
                         'fs__k': 500,
                         'union__pos__transf__min_df': 10,
                         'clf__C': 8.5316785241728138e-06,
